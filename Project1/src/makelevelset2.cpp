...
static float point_segment_distance(const Vec2f &x0, const Vec2f &x1, const Vec2f &x2)
{
   Vec2f dx(x2-x1);
   double m2=mag2(dx);
...
static void check_neighbour(const std::vector<Vec2ui> &edge, const std::vector<Vec2f> &x,
                            Array2f &phi, Array2i &closest_edge,
                            const Vec2f &gx, int i0, int j0, int i1, int j1)
{
   if(closest_edge(i0,j0)==closest_edge(i1,j1))
      return;
   if(closest_edge(i1,j1)>=0){
      unsigned int p, q; assign(edge[closest_edge(i1,j1)], p, q);
      float d=point_segment_distance(gx, x[p], x[q]);
      if(d<phi(i0,j0)){
         phi(i0,j0)=d;
                     ...
                     Array2f &phi)
{
   ...
   for(unsigned int e=0; e<edge.size(); ++e){
      ...
      for(int j=j0; j<=j1; ++j) for(int i=i0; i<=i1; ++i){
         Vec2f gx(i*dx+origin[0], j*dx+origin[1]);
         float d=point_segment_distance(gx, x[p], x[q]);
         if(d<phi(i,j)){
            phi(i,j)=d;
            closest_edge(i,j)=e;
